...
def temp_gas_run(gasses, Tlist, time_list, calibration_uid, background_uid,
    ...
    def inner():
        ...
        yield from abs_set(euro_ramp_rate, 100, wait=False)
        yield from abs_set(eurotherm, 700, wait=False)

        # Run shots until we hit one hour
        ...
        # ramp to rt
        yield from abs_set(euro_ramp_rate, 100, wait=False)
        yield from abs_set(eurotherm, 295, wait=False)
